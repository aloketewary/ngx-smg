...
import { getImportsType, getType, isNullOrUndefined, titleCase, getServiceType } from '../utils/smg.util';
import { SwaggerConfig } from './../models/swagger-config.model';
import { SwaggerSchemasModel } from './../models/swagger.schemas.model';
import { SwaggerServicePaths, ServiceOperationType, ServiceParameter } from '../models/swagger-paths.model';
...
`).replace(/"/g, '').replace(/:/g, ': ')}
  ...
  async createAbstractConfigFile(baseUrl: string, swaggerConfigData: SwaggerConfig, jsonData: any): string {
      ...
      Object.keys(paths).map((key: string) => {
        if (elem.name === key.split('/')[1]) {
          Object.keys(paths[key]).forEach(lastKey => {
            paths[key][lastKey]['url'] = key.replace(/{/g, "' + ").replace(/\}/g, " + '")
          })
